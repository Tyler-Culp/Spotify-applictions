...
def playlists():
    ...
    # topSongs = sp.current_user_top_tracks()
    # return topSongs
    cache_handler = spotipy.cache_handler.FlaskSessionCacheHandler(session)
    ...
    spotify = spotipy.Spotify(auth_manager=auth_manager)
    topSongsJson = spotify.current_user_top_tracks(time_range='long_term', limit=25)
    songList = []
    for i in range(len(topSongsJson["items"])):
        songList.append(topSongsJson["items"][i]["name"])
    songListString = ""
    for i in range(len(songList)):
        songListString = songListString + str(i+1) + ": " + songList[i] + " // " + topSongsJson['items'][i]['album']['artists'][0]['name'] + "<br>"
